   ...
   ui->label_EMAEvent->setText(QString("mEMA_EVENT_TYPE_GPI1_READ=%1").arg(m_gpi1value));
	...
	if (cmi_createCMIris(CMI_DMX_EMX_MODEL, &m_cmiHandle) == CMI_SUCCESS) {
		...
		if (m_eventThread) {
            ...
            connect(m_eventThread, SIGNAL(faceDetected(CMI_IMAGE_INFO*)),
                    this, SLOT(doFaceDetected(CMI_IMAGE_INFO *)));
		}
	}
	else {
...
{
    /*
    QSettings settings("CMIRISDemo.ini", QSettings::IniFormat);
    settings.beginGroup("MyGroup");
    settings.setValue("UIState", saveState(0));
    settings.endGroup();
    */
...
void MainWindow::recog() {
    ...
    ui->label_LeftArea->setText("");
    ui->label_RightArea->setText("");
    ...
    if (ui->checkBox_EnableVocalCues->isChecked()) {
    ...
    cmi_setFacePreviewEnabled(m_cmiHandle, CMI_TRUE);
...
void MainWindow::enroll() {
    ...
    // Set which eye to capture
    if (cmi_setWhichEye(m_cmiHandle, CMI_BOTH_EYES) != CMI_SUCCESS) {
        ui->statusBar->showMessage("Error in cmi_setWhichEye()!");
        return;
    }
...
bool MainWindow::canEnroll(CMI_IMAGE_INFO *imageInfo) {
    QMessageBox msgBox;

    CMI_MIR_DUAL_EYE_TEMPLATE matchTemplate;

    matchTemplate.left.templateData = new unsigned char [CMI_MIR_MATCH_ROT_13_TEMPLATE_SIZE];
    matchTemplate.left.templateSize = CMI_MIR_MATCH_ROT_13_TEMPLATE_SIZE;
    matchTemplate.right.templateData = new unsigned char [CMI_MIR_MATCH_ROT_13_TEMPLATE_SIZE];
    matchTemplate.right.templateSize = CMI_MIR_MATCH_ROT_13_TEMPLATE_SIZE;

    CMI_MIR_DUAL_EYE_SEGMENTATION dualEyeSegmentationInfo;
    int ret = cmi_mir_getMatchTemplates(m_cmiHandle, &matchTemplate, &dualEyeSegmentationInfo,
                                        imageInfo, CMI_MIR_SHOW_SEGMENTATION);
    ...
    CMI_MIR_DUAL_EYE_TEMPLATE enrolledTemplate;
    CMI_MIR_COMPARISON_RESULTS lresultInfo, rresultInfo;
    ...
    if((dualEyeSegmentationInfo.left.usableIrisArea < 70) || (dualEyeSegmentationInfo.right.usableIrisArea < 70)) {
#if defined(__linux__)
        system(QString("aplay %1/notRecognized.wav").arg(m_curPath).toStdString().c_str());
#else
        QSound::play("./notRecognized.wav");
#endif
        msgBox.setFont(QFont("Calibri", 26));
        msgBox.setText("Please wide open your eyes and try again!");
        msgBox.exec();
        ui->statusBar->showMessage(QString("Error! Usable iris area is less than 70 %1").arg(ret));
        delete [] matchTemplate.left.templateData;
        delete [] matchTemplate.right.templateData;
        return false;
    }

    ui->label_LeftArea->setText(QString("%1%").arg((int)dualEyeSegmentationInfo.left.usableIrisArea));
    ui->label_RightArea->setText(QString("%1%").arg((int)dualEyeSegmentationInfo.right.usableIrisArea));

    for (i = 0; i < m_database.recordListSize(); i++) {
        record = m_database.m_recordList.at(i);

        enrolledTemplate.left.templateData = (unsigned char *)record->leftIrisTemplate().data();
        enrolledTemplate.left.templateSize = CMI_MIR_MATCH_ROT_13_TEMPLATE_SIZE;
        enrolledTemplate.right.templateData = (unsigned char *)record->rightIrisTemplate().data();
        enrolledTemplate.right.templateSize = CMI_MIR_MATCH_ROT_13_TEMPLATE_SIZE;

        lret = cmi_mir_compareTemplate(m_cmiHandle, enrolledTemplate.left, matchTemplate.left, &lresultInfo);
        rret = cmi_mir_compareTemplate(m_cmiHandle, enrolledTemplate.right, matchTemplate.right, &rresultInfo);

        if ( lret == CMI_SUCCESS && rret != CMI_SUCCESS )
        {
            if ( lresultInfo.hammingDistance < m_hdThreshold )
            {
                minId = i;
            }
        }
        else if ( lret != CMI_SUCCESS && rret == CMI_SUCCESS )
        {
            if ( rresultInfo.hammingDistance < m_hdThreshold )
            {
                minId = i;
            }
        }
    ...
    if (minId == -1) {
            ...
            for (j = 0; j < height; j++) {
                for (i = 0; i < width; i++) {
                    if (i%2 == 0 && j%2 == 0) {
                        ...
                        m_leftQimage->setPixel(i/2, j/2+240, dbFaceImage->pixel(i, j));
                    }
                }
            }
        ...
        ui->label_Name->setText(QString("%1").arg(record->name()));
        ui->label_Result->setText(QString("Enrolled Iris"));
        ...
        //if (ok = dialogoverwrite.exec()) {  //Overwrite DB
            ...
            enrolTemplate.right.templateData = new unsigned char [CMI_MIR_ENROL_TEMPLATE_SIZE];
            enrolTemplate.right.templateSize = CMI_MIR_ENROL_TEMPLATE_SIZE;

            CMI_MIR_DUAL_EYE_SEGMENTATION dualEyeSegmentationInfo;
            int enrollRet = cmi_mir_getEnrolTemplates(m_cmiHandle, &enrolTemplate, &dualEyeSegmentationInfo,
                                                       imageInfo, CMI_MIR_SHOW_SEGMENTATION);

            if (enrollRet != CMI_SUCCESS) {
#if defined(__linux__)
                system(QString("aplay %1/notRecognized.wav").arg(m_curPath).toStdString().c_str());
#else
                QSound::play("./notRecognized.wav");
            ...
            QByteArray qba;
            qba = QByteArray((const char *)enrolTemplate.left.templateData, CMI_MIR_ENROL_TEMPLATE_SIZE);
            record->setLeftIrisTemplate(qba);

            qba = QByteArray((const char *)enrolTemplate.right.templateData, CMI_MIR_ENROL_TEMPLATE_SIZE);
            ...
            ///////////////////////////////////////////////////////////////////////////////////////
            //<Delete old face image & Save new face imgae>/////////////////////////////////////////
            ///////////////////////////////////////////////////////////////////////////////////////
            ...
            if (!imageDir.exists()) {
             ...
             QProcess process;
#if defined(__linux__)
            if(QFile::exists(record->leftIrisPath())) {
                process.start(QString("rm %1").arg(record->leftIrisPath()));
                process.waitForFinished(-1);
            }
            if(QFile::exists(record->rightIrisPath())) {
                process.start(QString("rm %1").arg(record->rightIrisPath()));
                process.waitForFinished(-1);
            }
            if(QFile::exists(record->faceImagePath())) {
                process.start(QString("rm %1").arg(record->faceImagePath()));
                process.waitForFinished(-1);
            }
#else
            QStringList qlist;
            ...
            if(QFile::exists(record->leftIrisPath())) {
                qlist = record->leftIrisPath().split("/");
                filepath = "";
                ...
                filename = "";
                filename += qlist.at(qlist.size()-1);
                process.start(QString("cmd /c del \"%1%2\"").arg(filepath).arg(filename));
                process.waitForFinished(-1);
            }

            if(QFile::exists(record->rightIrisPath())) {
                qlist = record->rightIrisPath().split("/");
                filepath = "";
                for(i=0; i<qlist.size()-1; i++) {
                    filepath += qlist.at(i);
                    filepath += "\\";
            ...
            if(QFile::exists(record->faceImagePath())) {
                qlist = record->faceImagePath().split("/");
                filepath = "";
                for(i=0; i<qlist.size()-1; i++) {
                    filepath += qlist.at(i);
                    filepath += "\\";
    ...
    #endif
                ui->statusBar->showMessage(QString("%1 was overwritten!").arg(record->name()));
                ui->label_Result->setText("Overwrite Success!");
            }
            ...
            QSound::play("./notRecognized.wav");
#endif
            ui->statusBar->showMessage(QString("%1 was not overwritten!").arg(record->name()));
        }
    }
    return false;
}
...
void MainWindow::doEnroll(CMI_IMAGE_INFO *imageInfo) {
    ...
    if (imageInfo->leftImage == NULL && imageInfo->rightImage == NULL) {
        ui->statusBar->showMessage("Error! left & right eye images null!");
		return;
	}
    ...
    enrolltemplate.right.templateData = new unsigned char [CMI_MIR_ENROL_TEMPLATE_SIZE];
    enrolltemplate.right.templateSize = CMI_MIR_ENROL_TEMPLATE_SIZE;

    CMI_MIR_DUAL_EYE_SEGMENTATION dualEyeSegmentationInfo;
    int ret = cmi_mir_getEnrolTemplates(m_cmiHandle, &enrolltemplate, &dualEyeSegmentationInfo,
                                        imageInfo, CMI_MIR_SHOW_SEGMENTATION);
...
#endif
            ui->statusBar->showMessage(QString("Error! Fail to get enroltemplate %1").arg(ret));
            delete [] enrolltemplate.left.templateData;
            delete [] enrolltemplate.right.templateData;
            return;
    }
    ...
    if (ok && !name.isEmpty()) {
        ...
        record.setLeftIrisTemplate(qba);
        qba = QByteArray((const char *)enrolltemplate.right.templateData, CMI_MIR_ENROL_TEMPLATE_SIZE);
        record.setRightIrisTemplate(qba);
        ...
        if (!m_database.insert(record)) {
            ...
            //--
				...
				ui->label_Result->setText("Enroll Success!");
        }
    }
	else {
...
void MainWindow::doRecog(CMI_IMAGE_INFO *imageInfo) {
    ...
    if (ret == CMI_MIR_LEFT_TEMPLATE_GENERATED_ONLY) {
        if ( dualEyeSegmentationInfo.left.usableIrisArea < 50 && dualEyeSegmentationInfo.right.usableIrisArea < 50 )
        {
            ui->statusBar->showMessage("Error! Usable Iris Area is less than 50");
    #if defined(__linux__)
            system(QString("aplay %1/notRecognized.wav").arg(m_curPath).toStdString().c_str());
    #else
            QSound::play("./notRecognized.wav");
        ...
        for (i = 0; i < m_database.recordListSize(); i++) {
            record = m_database.m_recordList.at(i);
            enrolTemplate.left.templateData = (unsigned char *)record->leftIrisTemplate().data();
            enrolTemplate.left.templateSize = CMI_MIR_MATCH_ROT_13_TEMPLATE_SIZE;
            lret = cmi_mir_compareTemplate(m_cmiHandle, enrolTemplate.left, matchTemplate.left, &lresultInfo);

            if (lret == CMI_SUCCESS && lresultInfo.coOcclusion < 70) {
                if (lresultInfo.hammingDistance < minLeftHD) {
                    minLeftHD = lresultInfo.hammingDistance;
                    if (lresultInfo.hammingDistance < m_hdThreshold) {
                        minId = i;
                    }
                }
        ...
        for (i = 0; i < m_database.recordListSize(); i++) {
            record = m_database.m_recordList.at(i);
            enrolTemplate.right.templateData = (unsigned char *)record->rightIrisTemplate().data();
            enrolTemplate.right.templateSize = CMI_MIR_MATCH_ROT_13_TEMPLATE_SIZE;
            rret = cmi_mir_compareTemplate(m_cmiHandle, enrolTemplate.right, matchTemplate.right, &rresultInfo);

            if (rret == CMI_SUCCESS && rresultInfo.coOcclusion < 70) {
                if (rresultInfo.hammingDistance < minRightHD) {
                    minRightHD = rresultInfo.hammingDistance;
                    if (rresultInfo.hammingDistance < m_hdThreshold) {
                        minId = i;
                    }
                }
        ...
        if ( ui->radioButton_RecogEitherEye->isChecked() )
        {
            if ( dualEyeSegmentationInfo.left.usableIrisArea < 50 && dualEyeSegmentationInfo.right.usableIrisArea < 50 )
            {
                ui->statusBar->showMessage("Error! Both Usable Iris Area is less than 50");
        #if defined(__linux__)
                system(QString("aplay %1/notRecognized.wav").arg(m_curPath).toStdString().c_str());
        #else
                QSound::play("./notRecognized.wav");
        ...
        {
            if ( dualEyeSegmentationInfo.left.usableIrisArea < 50 )
            {
                if ( dualEyeSegmentationInfo.right.usableIrisArea < 50 )
                {
                    ui->statusBar->showMessage("Error! Both Usable Iris Area is less than 50");
            #if defined(__linux__)
                    system(QString("aplay %1/notRecognized.wav").arg(m_curPath).toStdString().c_str());
                ...
                {
                    ui->statusBar->showMessage("Error! Left Usable Iris Area is less than 50");
            #if defined(__linux__)
                    system(QString("aplay %1/notRecognized.wav").arg(m_curPath).toStdString().c_str());
            #else
                    QSound::play("./notRecognized.wav");
            ...
            else
            {
                if ( dualEyeSegmentationInfo.right.usableIrisArea < 50 )
                {
                    ui->statusBar->showMessage("Error! Right Usable Iris Area is less than 50");
            #if defined(__linux__)
                    system(QString("aplay %1/notRecognized.wav").arg(m_curPath).toStdString().c_str());
            #else
                    QSound::play("./notRecognized.wav");
        ...
        for (i = 0; i < m_database.recordListSize(); i++) {
            ...
            if (lret == CMI_SUCCESS && lresultInfo.hammingDistance < minLeftHD) minLeftHD = lresultInfo.hammingDistance;
            if (rret == CMI_SUCCESS && rresultInfo.hammingDistance < minRightHD) minRightHD = rresultInfo.hammingDistance;

            if (lret == CMI_SUCCESS || rret == CMI_SUCCESS ) {
                if ((lret == CMI_SUCCESS && lresultInfo.hammingDistance < m_hdThreshold) ||
                        (rret == CMI_SUCCESS && rresultInfo.hammingDistance < m_hdThreshold)) {
                    // match found
                    if (minId == -1) {
                        minId = i;
                        if (lret == CMI_SUCCESS && lresultInfo.coOcclusion < 70) minLeftHD = lresultInfo.hammingDistance;
                        else minLeftHD = 0.5;
                        if (rret == CMI_SUCCESS && rresultInfo.coOcclusion < 70) minRightHD = rresultInfo.hammingDistance;
                        else minRightHD = 0.5;
                        minSumHD = minLeftHD + minRightHD;
                    }
                    else {
                        if (lresultInfo.hammingDistance + rresultInfo.hammingDistance < minSumHD) {
                            minId = i;
                            if (lret == CMI_SUCCESS && lresultInfo.coOcclusion < 70) minLeftHD = lresultInfo.hammingDistance;
                            else minLeftHD = 0.5;
                            if (rret == CMI_SUCCESS && rresultInfo.coOcclusion < 70) minRightHD = rresultInfo.hammingDistance;
    ...
    if (minId == -1) {
        ...
        ui->label_Result->setText("Iris Fail");
        ...
        ui->label_Result->setText("Iris Pass");
...
void MainWindow::displayLiveImages(CMI_IMAGE_INFO *imageInfo) {
    ...
    if (image) {
        if (ui->checkBox_Display->isChecked()) {
            // live image size is always less than 640x480
            x1 = m_rightQimage->width() - width;
            y1 = (DEMO_EYE_DISPLAY_HEIGHT - height);
            if (y1 < 0) return;

            for (j = 0; j < height; j++) {
                k = j*l * width*l;
                for (i = 0; i < width; i++) {
                    m_rightQimage->setPixel(i+x1, j+y1, TRIPLE(image[k + i*l]));
                }
            }
            m_rightPixmapItem->setPixmap(pixmap.fromImage(*m_rightQimage));
            ui->graphicsView_RightEye->show();
                ...
                for (j = 0; j < height2; j++) {
                    ...
                    for (i = 0; i < width2; i++) {
                        ...
                        if (i%2 == 0 && j%2 == 0) {
                            ...
                            m_rightQimage->setPixel(i/2+460, j/2+240,
                                  qRgb(colorImage[ind], colorImage[ind+1], colorImage[ind+2]));
                        }
                    }
                }
                ...
                for (j = 0; j < height; j+=2) {
                    ...
                    for (i = 0; i < width; i+=2) {
                        ...
                        if (i%4 == 0 && j%4 == 0) {
                            ...
                            m_rightQimage->setPixel(i/4+460, j/4+240, TRIPLE(image[k+i]));
                        }
                    }
                }
            }
            ...
            for (j = 0; j < height; j++) {
                ...
                for (i = 0; i < width; i++) {
                    ...
                    if (i%2 == 0 && j%2 == 0) {
                        ...
                        m_rightQimage->setPixel(i/2+460, j/2+240, TRIPLE(image[k+i]));
                    }
                }
            }
        }
