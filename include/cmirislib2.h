...
//  - cmi_setFaceCaptureMode(), cmi_getFaceCaptureMode() were added for color face image
//  - Minor bugs were fixed
//  - cmi_getMotorReturnToInitEnabled() and cmi_setMotorReturnToInitEnabled() were added
//
// Version 2.1.3   Oct. 31, 2014
// EMX family:
...
//  - New Windows driver supporting Windows 8 & 8.1 was included.
//  - fixed bugs in the driver and SDK when USB device is removed and arrives.
//
// Version 2.2.5   Dec. 30, 2014
//  - The error log file "cmirislog.txt" will be saved in the current directory for Windows
//    and in the directory /usr/local/share/CMITECH/ for ARM Linux
//  - The lockup problem when the USB communication is unstable was fixed.
//    ("No image to process error!" in cmirislog_[SerialNumber].txt)
//  - Version number has been unified.
//    (V1.2.5, V2.2.5)
//  - modified cmi_mir_getEnrolTemplates(), cmi_mir_getMatchTemplates() to get usable iris area.
//  - modified cmi_mir_compareTemplate() to get co-occlusion.
//  - IMPORTANT: cmi_mir_getEnrolTemplates(), cmi_mir_getMatchTemplates(),
//               and cmi_mir_compareTemplate() are not backward-compatible.
//  - Face preview images were added and it can be enabled by cmi_setFacePreviewEnabled();
//  - Detailed logs including usb transfer erros was added.
//
// Version 2.2.6   Feb. 17, 2015
//  - fixed buffer overflow bug in the BMTDemo when images are captured
//
// Version 2.2.8   June. 22, 2015
//  - added cmi_setHaarCascadePath() to set the folder where
//    the file "haarcascade_frontalface_alt.xml" is created for Java applet.
//  - fixed the problem when cmi_startCapture() is called immediately after cmi_stopCapture().
//
// Version 2.2.9   July. 03, 2015
//  - motion detection feature is added. The motion detection threshold can be set by
//    cmi_setMotionDetectionThreshold(). The default is motion detection feature off
//    (a value of motion detection threshold is 0) to maintain the backward compatibility.
//    If the motion detection threshold is large, it is less sensitive to the motion.
//    The recommended value is 10 and should not bigger than 40 and not less than 5.
//    If this feature is set with cmi_setMotionDetectonThreshold(), then PSD
//    (position sensitive device) in EMA or EMB will be disabled automatically.
//    If firmware version is less than v1.2.28 and motion detection threshold is set
//    as non-zero value, then cmi_openDevice() and/or cmi_setMotionDetectionThreshold()
//    will return CMI_ERROR_FIRMWARE_VERSION_INVALID. If the motor up or down button
//    is pressed, the capture will start.
//  - If the motor up or down button is pushed, the capture will start.
//  - Face detection for EMX family is improved in the dark ambient light. To use this feature,
//    the firmware of the EMX/EMA must be upgraded to v1.2.28.
//
// Version 2.3.0   Dec. 16, 2015
//  - added BMTFlippedCheckEnabled option. If this option is diabled (CMI_FALSE)
//    isDeviceUpSideDown in CMI_IMAGE_INFO will be always CMI_FALSE. The default is enabled.
//  - Fixed a bug that does not set MotorReturnToInitEnabled correctly in
//    cmi_setMotorReturnToInitEnabled().
//  - Fixed a bug that the LED's may flicker continuously in very bright environment.
//  - Fixed a bug that face recognition is time out when user captures own iris at the same time.
//    Developer should skip the face recognition routine if cmi_stopCapture returns -13 in the routine.
//
...
#define CMIRISLIB_MINOR_VERSION         3
#define CMIRISLIB_REVISION              0
...
#define CMI_MIR_ERROR_WRONG_LEFT_TEMPLATE_SIZE      -1031
#define CMI_MIR_ERROR_WRONG_RIGHT_TEMPLATE_SIZE     -1032
#define CMI_MIR_ERROR_WRONG_NOTSAME_TEMPLATE_SIZE   -1033
#define CMI_MIR_ERROR_FAIL_TO_COMPARE_TEMPLATES     -1034
#define CMI_MIR_ERROR_EMPTY_ENROL_TEMPLATE          -1035

typedef struct _CMI_MIR_TEMPLATE
{
    unsigned char* templateData;
    int templateSize;
}CMI_MIR_TEMPLATE;

typedef struct _CMI_MIR_DUAL_EYE_TEMPLATE
{
    CMI_MIR_TEMPLATE left;
    CMI_MIR_TEMPLATE right;
}CMI_MIR_DUAL_EYE_TEMPLATE;

typedef struct _CMI_MIR_EYE_SEGMENTATION
{
    double usableIrisArea;
}CMI_MIR_EYE_SEGMENTATION;

typedef struct _CMI_MIR_DUAL_EYE_SEGMENTATION
{
    CMI_MIR_EYE_SEGMENTATION left;
    CMI_MIR_EYE_SEGMENTATION right;
}CMI_MIR_DUAL_EYE_SEGMENTATION;

typedef struct _CMI_MIR_COMPARISON_RESULTS
{
    double hammingDistance;
    double coOcclusion;
}CMI_MIR_COMPARISON_RESULTS;
...
extern "C" {
	...
	//
	// handle[in] - CMI_HANDLE value
    // pdeviceInfo[in] - one of CMI_DEVICE_INFO found by cmi_readDMEvent()
	//
	// Return value - CMI_ERROR_INVALID_HANDLE
    ...
    int _CMI_EXPORT cmi_mir_getEnrolTemplates(CMI_HANDLE handle, CMI_MIR_DUAL_EYE_TEMPLATE* enrolTemplate,
                                              CMI_MIR_DUAL_EYE_SEGMENTATION* dualEyeSegmentationInfo,
                                              CMI_IMAGE_INFO *imageInfo, int showSegmentation);
    // Get enroll templates generated by MIRLIN Algorithm
    //
    // handle[in] - CMI_HANDLE value
    // enrolTemplate->left.templateData[out] - 579 bytes template
    // enrolTEmplate->left.templateSize[in] - should be CMI_MIR_ENROL_TEMPLATE_SIZE = 579
    // enrolTemplate->right.templateData[out] - 579 bytes template
    // enrolTEmplate->right.templateSize[in] - should be CMI_MIR_ENROL_TEMPLATE_SIZE = 579
    // dualEyeSegmentationInfo->left.usableIrisArea[out] - how much iris area is usable. If all iris area is usable
    //                                                    the value is 100.00(double) and it is 0.00(double) if all iris area isn't usable.
    // dualEyeSegmentationInfo->right.usableIrisArea[out] - how much iris area is usable. If all iris area is usable
    //                                                    the value is 100.00(double) and it is 0.00(double) if all iris area isn't usable.
    ...
    int _CMI_EXPORT cmi_mir_getMatchTemplates(CMI_HANDLE handle, CMI_MIR_DUAL_EYE_TEMPLATE* matchTemplate,
                                              CMI_MIR_DUAL_EYE_SEGMENTATION* dualEyeSegmentationInfo,
                                              CMI_IMAGE_INFO* imageInfo, int showSegmentation);
    // Get match templates generated by MIRLIN Algorithm
    //
    // handle[in] - CMI_HANDLE value
    // matchTemplate->left.templateData[out] - 579 bytes template
    // matchTemplate->left.templateSize[in] - should be one of the following sizes. The default is 7479 for 13 rotations
    ...
    // matchTemplate->right.templateData[out] - 579 bytes template
    // matchTemplate->right.templateSize[in] - should be one of the following sizes. The default is 7479 for 13 rotations
    //                                       CMI_MIR_MATCH_ROT_0_TEMPLATE_SIZE        579   // 0 rotations
    //                                       CMI_MIR_MATCH_ROT_5_TEMPLATE_SIZE        2879  // 5 rotations
    //                                       CMI_MIR_MATCH_ROT_7_TEMPLATE_SIZE        4029  // 7 rotations
    //                                       CMI_MIR_MATCH_ROT_9_TEMPLATE_SIZE        5179  // 9 rotations
    ...
    int _CMI_EXPORT cmi_mir_compareTemplate(CMI_HANDLE handle, const CMI_MIR_TEMPLATE enrolTemplate,
                                            const CMI_MIR_TEMPLATE matchTemplate, CMI_MIR_COMPARISON_RESULTS* comparisonResults);
    // Compare enroll template with match template by MIRLIN Algorithm
    //
    // handle[in] - CMI_HANDLE value
    // enrolTemplate.templateData[in] - 579 bytes template
    // matchTemplate.templateData[in] - matchTemplateSize bytes template
    // matchTemplate.templateSize[in] - matchTemplate size, which should be one of match template sizes defined above.
    // comparisonResults->hammingDistance[out] - hamming distance between two templates. If both templates are perfectly matched
    //                                           the value is 0 and it is 0.5 if two templates are totally non-matched.
    //                                           The recommended threshold is 0.15
    // comparisonResults->coOcclusion[out] - co-occlusion between two templates.
    //
    // Additional Return value - CMI_MIR_ERROR_UNKNOWN_LICENSE: cannot read the license
    //                           CMI_MIR_ERROR_INVALID_LICENSE: no MIRLIN license in the device
    //                           CMI_MIR_ERROR_WRONG_TEMPLATE_SIZE: wrong template size
    ...
    // For C# Wrapper ////
    //////////////////////
    int _CMI_EXPORT cmi_mir_getEnrolTemplates_cs(CMI_HANDLE handle, unsigned char* leftEnrolTemplate,
                                                     unsigned char* rightEnrolTemplate, int templateSize,
                                                  double* leftUsableIrisArea, double* rightUsableIrisArea,
                                                  CMI_IMAGE_INFO *imageInfo, int showSegmentation);
    // For C# Wrapper
    // Get enroll templates generated by MIRLIN Algorithm
    //
    ...
    // leftUsableIrisArea[out] - how much iris area is usable. If all iris area is usable
    //                           the value is 100.00(double) and it is 0.00(double) if all iris area isn't usable.
    // rightUsableIrisArea[out] - how much iris area is usable. If all iris area is usable
    //                            the value is 100.00(double) and it is 0.00(double) if all iris area isn't usable.
    // imageInfo[in] - CMI_IMAGE_INFO structure returned from the event CMI_EVENT_IRIS_IMAGES_SELECTED
    // showSegmentation[in] - whether to show the iris and pupil segmentation on the images in imageInfo
    //                        CMI_MIR_DO_NOT_SHOW_SEGMENTATION or CMI_MIR_SHOW_SEGMENTATION
    ...
    int _CMI_EXPORT cmi_mir_getMatchTemplates_cs(CMI_HANDLE handle, unsigned char* leftMatchTemplate,
    ...
    int _CMI_EXPORT cmi_mir_compareTemplate_cs(CMI_HANDLE handle, unsigned char* enrolTemplateData, unsigned char* matchTemplateData,
                                            int matchTemplateSize, double* hammingDistance, double* coOcclusion);
    // For C# Wrapper
    // Compare enroll template with match template by MIRLIN Algorithm
    //
    // handle[in] - CMI_HANDLE value
    // enrolTemplateData[in] - 579 bytes template
    // matchTemplateData[in] - matchTemplateSize bytes template
    ...
    //                                           the value is 0 and it is 0.5 if two templates are totally non-matched.
    //                                           The recommended threshold is 0.15
    // coOcclusion[out] - co-occlusion between two templates.
    //
    // Additional Return value - CMI_MIR_ERROR_UNKNOWN_LICENSE: cannot read the license
    //                           CMI_MIR_ERROR_INVALID_LICENSE: no MIRLIN license in the device
    //                           CMI_MIR_ERROR_WRONG_TEMPLATE_SIZE: wrong template size
    ...
    int _CMI_EXPORT cmi_getFacePreviewEnabled(CMI_HANDLE handle, int *enabled);

    int _CMI_EXPORT cmi_setFacePreviewEnabled(CMI_HANDLE handle, int enabled);
    // Enable/disable the face preview images for EMX, EMA and EMB.
    // The default is disabled.
    //
    // handle[in] - CMI_HANDLE value
    // enabled[in/out] - CMI_TRUE, CMI_FALSE

    int _CMI_EXPORT cmi_getFaceDetectWaitTimeSec(CMI_HANDLE handle, int *faceDetectWaitTimeSec);

    int _CMI_EXPORT cmi_setFaceDetectWaitTimeSec(CMI_HANDLE handle, int faceDetectWaitTimeSec);
    // After this faceDetectWaitTimeSec elapsed,
    // the EMA-30 will be in stand by state and wait for trigger by PSD.
    //
    // handle[in] - CMI_HANDLE value
    // faceDetectWaitTimeSec[in/out] -

    int _CMI_EXPORT cmi_setHaarCascadePath(CMI_HANDLE handle, const char *path);
    // set the folder where the file "haarcascade_frontalface_alt.xml" is created for Java applet.
    // The default is application's current directory.
    //
    // handle[in] - CMI_HANDLE value
    // path[in] - path for saving "haarcascade_frontalface_alt.xml" file.

    int _CMI_EXPORT cmi_getMotionDetectionThreshold(CMI_HANDLE handle, int *motionDetectionThreshold);

    int _CMI_EXPORT cmi_setMotionDetectionThreshold(CMI_HANDLE handle, int motionDetectionThreshold);
    // Set/Get motion detection threshold.
    // The motion detection threshold can be set by cmi_setMotionDetectionThreshold(). The default is
    // motion detection feature off (a value of motion detection threshold is 0) to maintain the backward
    // compatibility. If the motion detection threshold is large, it is less sensitive to the motion.
    // The recommended value is 10 and should not bigger than 40 and not less than 5. If this feature
    // is set with cmi_setMotionDetectonThreshold(), then PSD (position sensitive device) in EMA or EMB
    // will be disabled automatically. If firmware version is less than v1.2.28 and motion detection
    // threshold is set as non-zero value, then cmi_openDevice() and/or cmi_setMotionDetectionThreshold()
    // will return CMI_ERROR_FIRMWARE_VERSION_INVALID. If the motor up or down button is pressed,
    // the capture will start.
    //
    // handle[in] - CMI_HANDLE value
    // motionDetectionThreshold[in/out] - If it not in the range of 5 to 40, CMI_ERROR_IN_ARGUMENTS will
    //                                    be returned. Default is 0 (off) and recommended is 10.
    //
    // Additional Return value - CMI_ERROR_FIRMWARE_VERSION_INVALID: when the firmware version is less than 1.2.28.

    int _CMI_EXPORT cmi_getBMTFlippedCheckEnabled(CMI_HANDLE handle, int *enabled);

    int _CMI_EXPORT cmi_setBMTFlippedCheckEnabled(CMI_HANDLE handle, int enabled);
    // Set/Get BMTFlippedCheckEnabled.
    // If this option is diabled (CMI_FALSE), isDeviceUpSideDown in CMI_IMAGE_INFO will be always CMI_FALSE.
    // This means that BMT will capture images even though the device is flipped. The default is enabled.
    //
    // handle[in] - CMI_HANDLE value
    // BMTFlippedCheckEnabled[in/out] - CMI_TRUE or CMI_FALSE.
